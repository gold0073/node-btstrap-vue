...
export default {
  ...
  methods: {
    async getContentDetail() {
      ...
      await this.$axios
        .get(
          "http://127.0.0.1:8800/mdb_contents?content_id=" +
    ...
    async deleteData() {
      ...
      await this.$axios
        .post(
          this.$microSeviceUrl + "/mdb_contents",
          qs.stringify({
      ...
      await this.$axios(
        {
          url: 'http://127.0.0.1:8800/mdb_contents?content_id='+  this.$route.params.contentId,
          method:'post',
        }).then( ret =>{
        console.log("results :" , ret);
