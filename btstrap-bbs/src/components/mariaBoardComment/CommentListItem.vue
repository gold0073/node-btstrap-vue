...
export default {
  ...
  async created(){
    await this.$axios(
      {
        url : this.$microSeviceUrl + '/mdb_boards_comments?comment_id='+ this.commentObj.comment_id +'&act_type=sub_comment_inquery',
        method:'get'
      }).then( ret =>{
  ...
  methods: {
    ...
    async comment_deleteData(){
      ...
      await  this.$axios.post( this.$microSeviceUrl + "/mdb_boards_comments", qs.stringify({
