...
export default {
  ...
  methods: {
    async getContentDetail() {
      ...
      await this.$axios
        .get(
          "http://127.0.0.1:8800/pgdb_contents?content_id=" +
            Number(this.$route.params.contentId) +
    ...
    async deleteData() {
      ...
      await this.$axios
        .post(
          this.$microSeviceUrl + "/pgdb_contents",
          qs.stringify({
      ...
      await this.$axios(
        {
          url: 'http://127.0.0.1:8800/pgdb_contents?content_id='+  this.$route.params.contentId,
          method:'post',
        }).then( ret =>{
        console.log("results :" , ret);
